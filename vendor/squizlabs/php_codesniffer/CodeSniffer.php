...
if (!class_exists('PHP_CodeSniffer_Fixer', true)) {
...
if (!class_exists('PHP_CodeSniffer_CLI', true)) {
...
class PHP_CodeSniffer
{
     ...
     */
    protected static array $rulesetDirs = array();
     ...
     */
    public ?\PHP_CodeSniffer_Reporting $reporting = null;
     ...
     */
    public array $sniffCodes = array();

    /**
     * The listeners array, indexed by token type.
     */
    private array $_tokenListeners = array();
     ...
     */
    protected array $ignorePatterns = array();

    /**
     * An array of extensions for files we will check.
     */
    public array $allowedFileExtensions = array();
     ...
     */
    public array $defaultFileExtensions = array(
                                     'php' => 'PHP',
                                     'inc' => 'PHP',
                                     'js'  => 'JS',
                                     'css' => 'CSS',
     ...
     * @var array(string)
     */
    public static array $allowedTypes = array(
                                   'array',
                                   'boolean',
                                   'float',
                                   'integer',
    ...
    public function __construct(
        ...
        if (!defined('PHPCS_DEFAULT_ERROR_SEV')) {
            define('PHPCS_DEFAULT_ERROR_SEV', 5);
        }

        if (!defined('PHPCS_DEFAULT_WARN_SEV')) {
    ...
    {
        $newClassName = substr($className, 0, 4) === 'PHP_' ? substr($className, 4) : $className;

        $path = str_replace(array('_', '\\'), DIRECTORY_SEPARATOR, $newClassName).'.php';

        if (is_file(dirname(__FILE__).DIRECTORY_SEPARATOR.$path)) {
            ...
            foreach ($installedPaths as $installedPath) {
                if (is_file($installedPath.DIRECTORY_SEPARATOR.$path)) {
            ...
            foreach (self::$rulesetDirs as $rulesetDir) {
                if (is_file(dirname($rulesetDir).DIRECTORY_SEPARATOR.$path)) {
    ...
    {
        if (!defined('PHP_CODESNIFFER_TAB_WIDTH')) {
    ...
    {
        if (!defined('PHP_CODESNIFFER_ENCODING')) {
    ...
    {
        if (!defined('PHP_CODESNIFFER_INTERACTIVE')) {
    ...
    public function setAllowedFileExtensions(array $extensions)
    {
        ...
        foreach ($extensions as $ext) {
            ...
            if (isset($this->allowedFileExtensions[$ext])) {
                $newExtensions[$ext] = $this->allowedFileExtensions[$ext];
            } elseif (isset($this->defaultFileExtensions[$ext])) {
    ...
    public function initStandard($standards, array $restrictions=array(), array $exclusions=array())
    {
        ...
        if (defined('PHP_CODESNIFFER_IN_TESTS') && !empty($restrictions)) {
            ...
            if ($installed !== null) {
                ...
                if (is_dir($standard)
                    && is_file(self::realpath($standard.DIRECTORY_SEPARATOR.'ruleset.xml'))
                ) {
    ...
    public function processRuleset($rulesetPath, $depth=0)
    {
        ...
        if (is_dir($rulesetDir.DIRECTORY_SEPARATOR.'Sniffs')) {
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                echo str_repeat("\t", $depth);
        ...
        foreach ($ruleset->{'config'} as $config) {
            if (!$this->_shouldProcessElement($config)) {
                continue;
            }
        ...
        foreach ($ruleset->rule as $rule) {
            if (!isset($rule['ref'])
                || !$this->_shouldProcessElement($rule)
            ) {
                continue;
            }
            ...
            if (property_exists($rule, 'exclude') && $rule->exclude !== null) {
                foreach ($rule->exclude as $exclude) {
                    if (!$this->_shouldProcessElement($exclude)) {
                        continue;
                    }
        ...
        foreach ($ruleset->{'arg'} as $arg) {
            if (!$this->_shouldProcessElement($arg)) {
                continue;
            }
        ...
        if (empty($cliValues['files']) && $cliValues['stdin'] === null) {
        ...
        foreach ($ruleset->{'exclude-pattern'} as $pattern) {
            if (!$this->_shouldProcessElement($pattern)) {
                continue;
            }
        ...
        foreach ($includedSniffs as $sniff) {
            if (in_array($sniff, $excludedSniffs)) {
                continue;
            } else {
    ...
    private function _expandRulesetReference($ref, $rulesetDir, $depth=0)
    {
        ...
        if (is_file($ref)) {
            ...
            if (self::isPharFile($path) === true && strpos($path, 'ruleset.xml') === false) {
                ...
                if (file_exists($path.DIRECTORY_SEPARATOR.'ruleset.xml')) {
        ...
        if (is_dir($ref)) {
            if (is_file($ref.DIRECTORY_SEPARATOR.'ruleset.xml')) {
    ...
    private function _processRule($rule, $depth=0)
    {
        ...
        // Custom severity.
        if (property_exists($rule, 'severity') && $rule->severity !== null
            && $this->_shouldProcessElement($rule->severity)
        ) {
        ...
        // Custom message type.
        if (property_exists($rule, 'type') && $rule->type !== null
            && $this->_shouldProcessElement($rule->type)
        ) {
            if (!isset($this->ruleset[$code])) {
                $this->ruleset[$code] = array();
            }
        ...
        // Custom message.
        if (property_exists($rule, 'message') && $rule->message !== null
            && $this->_shouldProcessElement($rule->message)
        ) {
            if (!isset($this->ruleset[$code])) {
                $this->ruleset[$code] = array();
            }
        ...
        // Custom properties.
        if (property_exists($rule, 'properties') && $rule->properties !== null
            && $this->_shouldProcessElement($rule->properties)
        ) {
            foreach ($rule->properties->property as $prop) {
                if (!$this->_shouldProcessElement($prop)) {
                    continue;
                }
                ...
                } elseif (!isset($this->ruleset[$code]['properties'])) {
    ...
    {
        if (!isset($element['phpcbf-only'])
            && !isset($element['phpcs-only'])
        ) {
        ...
        if (PHP_CODESNIFFER_CBF === true
            && isset($element['phpcbf-only'])
            && (string) $element['phpcbf-only'] === 'true'
        ) {
            return true;
        }
        return PHP_CODESNIFFER_CBF === false
            && isset($element['phpcs-only'])
    ...
    public function registerSniffs($files, $restrictions, $exclusions)
    {
        ...
        foreach ($files as $file) {
            ...
            if (!empty($restrictions)
                && !in_array(strtolower($className), $restrictions)
            ) {
                continue;
            }
            ...
            // to see if this sniff is allowed.
            if (!empty($exclusions)
                && in_array(strtolower($className), $exclusions)
            ) {
                continue;
            }
    ...
    public function populateTokenListeners()
    {
        ...
        foreach ($this->sniffs as $listenerClass) {
            ...
            $parts = strstr($listenerClass, '\\') === false ? explode('_', $listenerClass) : explode('\\', $listenerClass);
            ...
            if (isset($this->ruleset[$code]['properties'])) {
            ...
            foreach ($tokens as $token) {
                if (!isset($this->_tokenListeners[$token])) {
                    $this->_tokenListeners[$token] = array();
                }

                if (!isset($this->_tokenListeners[$token][$listenerClass])) {
    ...
    public function getFilesToProcess($paths, $local=false)
    {
        ...
        foreach ($paths as $path) {
            if (is_dir($path) || self::isPharFile($path) === true) {
    ...
    public function shouldIgnoreFile($path, $basedir)
    {
        ...
        foreach ($this->ignorePatterns as $pattern => $type) {
            ...
            $testPath = $type === 'relative' ? $relativePath : $path;
    ...
    {
        if (!class_exists('PHP_CodeSniffer_DocGenerators_'.$generator, true)) {
    ...
    public static function suggestType($varType)
    {
        ...
        if (in_array($varType, self::$allowedTypes)) {
            return $varType;
        } else {
            ...
            if (strpos($lowerVarType, 'array(') !== false) {
                ...
                //end if
            } elseif (in_array($lowerVarType, self::$allowedTypes)) {
    ...
    public static function getInstalledStandards(
        ...
        $installedPaths = $standardsDir === '' ? self::getInstalledStandardPaths() : array($standardsDir);
        ...
        foreach ($installedPaths as $standardsDir) {
            ...
            foreach ($di as $file) {
                if ($file->isDir() && !$file->isDot()) {
                    ...
                    if (!$includeGeneric && $filename === 'Generic') {
                        continue;
                    }
    ...
    public static function isInstalledStandard($standard)
    {
        ...
        //end if
        return $path !== null && strpos($path, 'ruleset.xml') !== false;
    ...
    public static function getConfigData($key)
    {
        ...
        if (!isset($phpCodeSnifferConfig[$key])) {
            return null;
        }
...
?>';
    ...
    {
        if (isset($GLOBALS['PHP_CODESNIFFER_CONFIG_DATA'])) {
