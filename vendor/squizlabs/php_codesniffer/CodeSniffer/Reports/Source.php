...
class PHP_CodeSniffer_Reports_Source implements PHP_CodeSniffer_Report
{
        ...
        foreach ($report['messages'] as $line => $lineErrors) {
            foreach ($lineErrors as $column => $colErrors) {
                foreach ($colErrors as $error) {
                    $source = $error['source'];
                    if (!isset($this->_sourceCache[$source])) {
                        if ($showSources) {
                            $parts = null;
                            $sniff = $source;
                        } else {
    ...
    public function generate(
        ...
        $width = $showSources ? min($width, ($maxLength + 11)) : min($width, ($maxLength + 41));
        ...
        $maxSniffWidth = $showSources ? $width - 7 : $width - 37;
