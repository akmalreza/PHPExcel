...
abstract class PHP_CodeSniffer_Reports_VersionControl implements PHP_CodeSniffer_Report
{
     ...
     */
    private array $_authorCache = array();

    /**
     * A cache of blame stats collected during the run.
     */
    private array $_praiseCache = array();

    /**
     * A cache of source stats collected during the run.
     */
    private array $_sourceCache = array();
    ...
    public function generateFileReport(
        ...
        foreach ($report['messages'] as $line => $lineErrors) {
            ...
            if (!isset($this->_authorCache[$author])) {
            ...
            foreach ($lineErrors as $column => $colErrors) {
                foreach ($colErrors as $error) {
                    ...
                    if ($showSources) {
                        $source = $error['source'];
                        if (!isset($this->_sourceCache[$author][$source])) {
    ...
    public function generate(
        ...
        $maxLength = 0;
        foreach (array_keys($this->_authorCache) as $author) {
            $maxLength = max($maxLength, strlen($author));
            if ($showSources && isset($this->_sourceCache[$author])) {
        ...
        if ($toScreen && PHP_CODESNIFFER_INTERACTIVE === false) {
