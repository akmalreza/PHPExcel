...
class PHP_CodeSniffer_Tokenizers_PHP
{
     ...
     */
    public bool $skipMinified = false;
     ...
     */
    public array $scopeOpeners = array(
                            T_IF            => array(
                                                'start'  => array(
     ...
     */
    public array $endScopeTokens = array(
                              T_CLOSE_CURLY_BRACKET => T_CLOSE_CURLY_BRACKET,
                              T_ENDIF               => T_ENDIF,
                              T_ENDFOR              => T_ENDFOR,
                              T_ENDFOREACH          => T_ENDFOREACH,
     ...
     */
    private static array $_resolveTokenCache = array();
    ...
    public function tokenizeString($string, $eolChar='\n')
    {
        ...
        for ($stackPtr = 0; $stackPtr < $numTokens; $stackPtr++) {
            ...
            */
            ...
            if ($tokenIsArray && substr($token[1], -1) === "\r" && (isset($tokens[($stackPtr + 1)])
                && is_array($tokens[($stackPtr + 1)])
            ...
            */
            ...
            if ($tokenIsArray && $token[0] === T_DOC_COMMENT) {
            ...
            */
            ...
            if (!$tokenIsArray && ($token[0] === '"' || $token[0] === 'b"')) {
            ...
            */
            ...
            if ($tokenIsArray && $token[0] === T_START_HEREDOC) {
            ...
            */
            ...
            if (!$tokenIsArray && $token[0] === '?') {
                $newToken            = array();
                $newToken['content'] = '?';

                $prevNonEmpty = null;
                for ($i = ($stackPtr - 1); $i >= 0; $i--) {
                    $tokenType = is_array($tokens[$i]) ? $tokens[$i][0] : $tokens[$i];

                    if ($prevNonEmpty === null
                        && !isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokenType])
                    ) {
                    ...
                    if ($tokenType === T_FUNCTION) {
                        ...
                        $newToken['type'] = 'T_NULLABLE';
                        break;
                    } elseif (in_array($tokenType, array(T_OPEN_TAG, T_OPEN_TAG_WITH_ECHO, '=', '{', ';'))) {
            ...
            */
            ...
            if ($stackPtr > 1
                ...
                && $token[0] !== T_VARIABLE
                && $token[0] !== T_DOLLAR
                && !isset(PHP_CodeSniffer_Tokens::$emptyTokens[$token[0]])
            ) {
                $newToken            = array();
            ...
            */
            ...
            if ($tokenIsArray && $token[0] === T_FUNCTION) {
                for ($x = ($stackPtr + 1); $x < $numTokens; $x++) {
                    if (!is_array($tokens[$x])
                        || !isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$x][0]])
                    ) {
                        // Non-empty content.
                        break;
                    }
                ...
                if ($x < $numTokens && is_array($tokens[$x])) {
                    $tokens[$x][0] = T_STRING;
                }
            }
            ...
            */
            ...
            if ($tokenIsArray
                ...
                for ($x = ($newStackPtr - 1); $x > 0; $x--) {
                    if (isset($stopTokens[$finalTokens[$x]['code']])) {
                        break;
                    }
                }
            ...
            */
            ...
            if ($tokenIsArray && token_name($token[0]) === 'T_HASHBANG') {
                $finalTokens[$newStackPtr] = array(
                                              'content' => $token[1],
                                              'code'    => T_INLINE_HTML,
                                              'type'    => 'T_INLINE_HTML',
            ...
            */
            ...
            if ($tokenIsArray && strpos($token[1], $eolChar) !== false) {
                ...
                if ($tokenIsArray && $token[0] === T_STRING) {
                    ...
                    if (isset($context[$finalTokens[$lastNotEmptyToken]['code']])) {
                ...
                $newToken = null;
                if (!$tokenIsArray) {
                    if (isset(self::$_resolveTokenCache[$token[0]])) {
                    ...
                    if ($cacheKey !== null && isset(self::$_resolveTokenCache[$cacheKey])) {
                ...
                // inline IF statement.
                if (!empty($insideInlineIf) && $newToken['code'] === T_COLON) {
                ...
                if ($insideUseGroup && $newToken['code'] === T_CLOSE_CURLY_BRACKET) {
    ...
    public function processAdditional(&$tokens, $eolChar)
    {
        ...
        for ($i = ($numTokens - 1); $i >= 0; $i--) {
            ...
            if (isset($tokens[$i]['scope_opener'])
            ...
            if ($tokens[$i]['code'] === T_FUNCTION) {
                ...
                if (isset($tokens[$i]['scope_opener'])) {
                    ...
                    if ($tokens[$x]['code'] === T_OPEN_PARENTHESIS) {
                        ...
                        for ($x = ($tokens[$i]['scope_opener'] + 1); $x < $tokens[$i]['scope_closer']; $x++) {
                            if (!isset($tokens[$x]['conditions'][$i])) {
                                continue;
                            }
                ...
                */
                for ($x = ($tokenAfterReturnTypeHint - 1); $x > $i; $x--) {
                    if (!isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$x]['code']])) {
                        if (in_array($tokens[$x]['code'], array(T_STRING, T_ARRAY, T_ARRAY_HINT, T_CALLABLE, T_SELF, T_PARENT), true)) {
                            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                $line = $tokens[$x]['line'];
                                $type = $tokens[$x]['type'];
                ...
                continue;
            } elseif ($tokens[$i]['code'] === T_CLASS && isset($tokens[$i]['scope_opener'])) {
                /*
                ...
                */
                ...
                for ($x = ($i - 1); $x > 0; $x--) {
                    ...
                    if (isset($tokens[$x]['scope_opener'])) {
                        $isShortArray = true;
                        break;
                    }

                    if (!isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$x]['code']])) {
                        if (!isset($allowed[$tokens[$x]['code']])) {
                            $isShortArray = true;
                        }
                ...
                if (isset($context[$tokens[$x]['code']])) {
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                        $line = $tokens[$i]['line'];
                        $type = $tokens[$i]['type'];
            ...
            if ($tokens[$x]['code'] !== T_OPEN_CURLY_BRACKET
                || isset($tokens[$x]['scope_condition'])
            ) {
