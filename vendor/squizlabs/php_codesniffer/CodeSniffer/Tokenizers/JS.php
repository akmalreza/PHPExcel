...
class PHP_CodeSniffer_Tokenizers_JS
{
     ...
     */
    protected array $tokenValues = array(
                              'function'  => 'T_FUNCTION',
     ...
     */
    protected array $stringTokens = array(
                               '\'' => '\'',
                               '"'  => '"',
                              );
     ...
     */
    protected array $commentTokens = array(
                                '//'  => null,
                                '/*'  => '*/',
                                '/**' => '*/',
                               );
    ...
    public function tokenizeString($string, $eolChar='\n')
    {
        ...
        for ($i = 0; $i < $numChars; $i++) {
            ...
            // Process strings.
            if ($inComment === '' && isset($this->stringTokens[$char])) {
                if ($inString === $char) {
            ...
            if (isset($this->tokenValues[strtolower($buffer)])
                ...
                if ($lookAheadLength > 0) {
                    ...
                    for ($x = 1; $x <= $lookAheadLength; $x++) {
                        ...
                        if (isset($this->tokenValues[strtolower($charBuffer)])) {
                ...
                //end if
            } elseif (isset($this->tokenValues[strtolower($char)])) {
            ...
            if ($inComment === ''
                && array_key_exists($buffer, $this->commentTokens)) {
                ...
                if (PHP_CODESNIFFER_VERBOSITY > 1 && $inComment === '') {
                    echo "\t\t* found end of comment *".PHP_EOL;
                }
                if ($inComment === '' && !$cleanBuffer) {
                    $tokens[] = array(
                                 'code'    => T_STRING,
                                 'type'    => 'T_STRING',
                                 'content' => str_replace("\n", $eolChar, $buffer),
        ...
        for ($stackPtr = 0; $stackPtr < $numTokens; $stackPtr++) {
            ...
            */
            ...
            if ($token['code'] === T_COMMENT || $token['code'] === T_DOC_COMMENT) {
                ...
                $endContent = null;
                if (isset($this->commentTokens[$tokenContent])) {
    ...
    public function getRegexToken($char, $string, $chars, $tokens, $eolChar)
    {
        ...
        for ($prev = ($numTokens - 1); $prev >= 0; $prev--) {
            if (!isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$prev]['code']])) {
                break;
            }
        }

        if (!isset($beforeTokens[$tokens[$prev]['code']])) {
            return null;
        }
        ...
        if (!isset($afterTokens[$chars[$next]])) {
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
    ...
    public function processAdditional(&$tokens, $eolChar)
    {
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            if ($tokens[$i]['code'] === T_FUNCTION && isset($tokens[$i]['scope_opener'])) {
                ...
                for ($label = ($i - 1); $label >= 0; $label--) {
                    if (!isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$label]['code']])) {
                        break;
                    }
                }
