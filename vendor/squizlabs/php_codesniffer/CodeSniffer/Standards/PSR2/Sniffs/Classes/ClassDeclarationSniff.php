...
if (!class_exists('PEAR_Sniffs_Classes_ClassDeclarationSniff', true)) {
    ...
    public function processOpen(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[($stackPtr - 1)]['code'] === T_WHITESPACE) {
            ...
            if ($prevContent !== $phpcsFile->eolChar) {
                ...
                if (in_array($tokens[($stackPtr - 2)]['code'], array(T_ABSTRACT, T_FINAL))
                    && $spaces !== 1
                ) {
        ...
        $keywordTokenType = $tokens[$stackPtr]['code'] === T_INTERFACE ? T_EXTENDS : T_IMPLEMENTS;
        ...
        foreach ($classNames as $i => $className) {
            ...
            if ($checkingImplements
                ...
                if ($prev === $implementsToken && $tokens[$className]['line'] !== ($tokens[$prev]['line'] + 1)) {
                    ...
                    $found = $tokens[$prev]['line'] !== $tokens[$className]['line'] ? 0 : strlen($tokens[$prev]['content']);
                ...
                if ($tokens[($className - 1)]['code'] === T_COMMA
                    ...
                    $prev = $tokens[($className - 1)]['code'] === T_NS_SEPARATOR ? $className - 2 : $className - 1;
            ...
            if ($checkingImplements
                ...
                if ($i !== ($classCount - 1) && $tokens[($className + 2)]['code'] !== $keywordTokenType) {
