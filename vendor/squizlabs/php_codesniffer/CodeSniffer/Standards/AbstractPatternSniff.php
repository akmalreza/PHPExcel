...
abstract class PHP_CodeSniffer_Standards_AbstractPatternSniff implements PHP_CodeSniffer_Sniff
{
     ...
     */
    private array $_parsedPatterns = array();
     ...
     */
    private array $_supplementaryTokens = array();
    ...
    public final function register()
    {
        ...
        foreach ($patterns as $pattern) {
            ...
            if (!isset($this->_parsedPatterns[$tokenType])) {
    ...
    private function _getPatternTokenTypes($pattern)
    {
        ...
        foreach ($pattern as $pos => $patternInfo) {
            if ($patternInfo['type'] === 'token' && !isset($tokenTypes[$patternInfo['token']])) {
    ...
    public final function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if (in_array($tokens[$stackPtr]['code'], $this->_supplementaryTokens)) {
        ...
        foreach ($this->_parsedPatterns[$type] as $patternInfo) {
            ...
            foreach ($errors as $stackPtr => $error) {
                if (!isset($this->_errorPos[$stackPtr])) {
            ...
            for ($i = ($patternInfo['listen_pos'] - 1); $i >= 0; $i--) {
                if ($pattern[$i]['type'] === 'token') {
                    if ($pattern[$i]['token'] === T_WHITESPACE) {
                        ...
                        $stackPtr = isset($pattern[($i - 1)])
                            && $pattern[($i - 1)]['type'] === 'skip' ? $prev : $prev - 1;
                    }
                    //end if
                    ...
                    if ($next === false || !isset($tokens[$next][$to])) {
                        // If there was not opener, then we must be
                        // using the wrong pattern.
                        return false;
                    }
                    $found = $to === 'parenthesis_opener' ? '{'.$found : '('.$found;
                    $found = '...'.$found;
                    ...
                    if ($this->ignoreComments
                        && isset(PHP_CodeSniffer_Tokens::$commentTokens[$tokens[$stackPtr]['code']])
                    ) {
                    ...
                    if ($tokens[$stackPtr]['code'] === T_WHITESPACE) {
                        if ($tokens[$stackPtr]['content'] !== $phpcsFile->eolChar) {
                            ...
                            if (($tokens[($stackPtr - 1)]['content'] !== $phpcsFile->eolChar)
                                && ($this->ignoreComments
                                && !isset(PHP_CodeSniffer_Tokens::$commentTokens[$tokens[($stackPtr - 1)]['code']]))
                            ) {
                                $hasError = true;
                            } else {
                                $stackPtr--;
                    ...
                    //end if
                    if (!$hasError && $pattern[($i - 1)]['type'] !== 'newline') {
            ...
            if ($pattern[$i]['type'] === 'token') {
                if ($pattern[$i]['token'] === T_WHITESPACE) {
                    if ($this->ignoreComments) {
                        ...
                        if (isset(PHP_CodeSniffer_Tokens::$commentTokens[$tokens[$stackPtr]['code']])) {
                            continue;
                        }
                    ...
                    if ($lastAddedStackPtr !== null) {
                        ...
                        if (($tokens[$next]['code'] === T_OPEN_PARENTHESIS
                            ...
                            && isset($tokens[$next]['parenthesis_owner'])
                    ...
                    if (($next - $stackPtr) > 0) {
                        ...
                        for ($j = $stackPtr; $j < $next; $j++) {
                            ...
                            if (isset(PHP_CodeSniffer_Tokens::$commentTokens[$tokens[$j]['code']])) {
                                $hasComment = true;
                            }
                        }
                    ...
                    $stackPtr = isset($pattern[($i + 1)])
                        && $pattern[($i + 1)]['type'] === 'skip' ? $next : $next + 1;
                }
                //end if
            } elseif ($pattern[$i]['type'] === 'skip') {
                if ($pattern[$i]['to'] === 'unknown') {
                    ...
                    if ($next === false
                        || !isset($tokens[$next][$pattern[$i]['to']])
                    ) {
                ...
                if ($newline === 0) {
                    ...
                    if (!$this->ignoreComments && isset(PHP_CodeSniffer_Tokens::$commentTokens[$tokens[$newline]['code']])) {
                        $hasError = true;
                    }
    ...
    private function _parse($pattern)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            if ($specialPattern !== false || $isLastChar) {
