...
class Squiz_Sniffs_PHP_CommentedOutCodeSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        */
        ...
        if (!isset($stringTokens[($numTokens - 1)])
        ...
        if ($phpcsFile->tokenizerType === 'PHP'
            && !isset(PHP_CodeSniffer_Tokens::$emptyTokens[$stringTokens[($numTokens - 2)]['code']])
        ) {
            return;
        }
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            if (isset($emptyTokens[$stringTokens[$i]['code']])) {
                // Looks like comment.
                $numComment++;
            } elseif (in_array($stringTokens[$i]['code'], PHP_CodeSniffer_Tokens::$comparisonTokens)
