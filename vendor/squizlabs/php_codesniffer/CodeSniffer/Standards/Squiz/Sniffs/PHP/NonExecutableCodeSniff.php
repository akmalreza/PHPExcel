    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = ($stackPtr - 1); $i > 0; $i--) {
            if ($tokens[$i]['code'] === T_CLOSE_PARENTHESIS) {
                ...
                continue;
            } elseif (isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$i]['code']])) {
                continue;
            }
        ...
        $prev = $phpcsFile->findPrevious(PHP_CodeSniffer_Tokens::$emptyTokens, ($stackPtr - 1), null, true);
        if (isset($tokens[$prev]['parenthesis_owner'])) {
            ...
            if (isset($ignore[$tokens[$owner]['code']])) {
                return;
            }
        }
        ...
        if (!empty($ourConditions)) {
            ...
            for ($i = ($stackPtr + 1); $i < $closer; $i++) {
                if (isset($tokens[$i]['scope_closer']) && $tokens[$i]['scope_closer'] === $closer) {
            ...
            $end = $nextOpener === null ? $closer : $nextOpener - 1;
        } else {
        ...
        for ($i = ($start + 1); $i < $end; $i++) {
            if (isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$i]['code']])
                || isset(PHP_CodeSniffer_Tokens::$bracketTokens[$tokens[$i]['code']])
            ) {
                continue;
            }
