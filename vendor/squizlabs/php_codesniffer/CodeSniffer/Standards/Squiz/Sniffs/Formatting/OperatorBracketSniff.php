...
class Squiz_Sniffs_Formatting_OperatorBracketSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === T_MINUS) {
            ...
            if ($tokens[$number]['code'] === T_LNUMBER || $tokens[$number]['code'] === T_DNUMBER) {
                ...
                if ($previous !== false) {
                    ...
                    if ($isAssignment || $isEquality || $isComparison) {
        ...
        if ($previousToken !== false) {
            ...
            if (in_array($tokens[$previousToken]['code'], $invalidTokens)) {
                return;
            }
        }
            ...
            foreach ($parenthesis as $bracket => $endBracket) {
                ...
                if ($prevCode === T_STRING || $prevCode === T_SWITCH) {
                    ...
                    for ($prev = ($stackPtr - 1); $prev > $bracket; $prev--) {
                        if (in_array($tokens[$prev]['code'], $allowed)) {
                            continue;
                        }
                    ...
                    for ($next = ($stackPtr + 1); $next < $endBracket; $next++) {
                        if (in_array($tokens[$next]['code'], $allowed)) {
                            continue;
                        }
                ...
                if (in_array($prevCode, PHP_CodeSniffer_Tokens::$scopeOpeners) && $prevCode !== T_SWITCH) {
                    break;
                }
    ...
    public function addMissingBracketsError(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for ($before = ($stackPtr - 1); $before > 0; $before--) {
            ...
            if (isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$before]['code']])
                || isset(PHP_CodeSniffer_Tokens::$operators[$tokens[$before]['code']])
                || isset(PHP_CodeSniffer_Tokens::$castTokens[$tokens[$before]['code']])
                || isset($allowed[$tokens[$before]['code']])
            ) {
                continue;
            }
        ...
        for ($after = ($stackPtr + 1); $after < $phpcsFile->numTokens; $after++) {
            ...
            if (isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$after]['code']])
                || isset(PHP_CodeSniffer_Tokens::$operators[$tokens[$after]['code']])
                || isset(PHP_CodeSniffer_Tokens::$castTokens[$tokens[$after]['code']])
                || isset($allowed[$tokens[$after]['code']])
            ) {
                continue;
            }
