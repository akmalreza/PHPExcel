...
class Squiz_Sniffs_Operators_ComparisonOperatorUsageSniff implements PHP_CodeSniffer_Sniff
{
     ...
     */
    private static array $_validOps = array(
                                 T_IS_IDENTICAL,
                                 T_IS_NOT_IDENTICAL,
                                 T_LESS_THAN,
                                 T_GREATER_THAN,
     ...
     */
    private static array $_invalidOps = array(
                                   'PHP' => array(
                                             T_IS_EQUAL     => '===',
                                             T_IS_NOT_EQUAL => '!==',
                                             T_BOOLEAN_NOT  => '=== FALSE',
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $start; $i <= $end; $i++) {
            $type = $tokens[$i]['code'];
            if (array_key_exists($type, self::$_invalidOps[$tokenizer])) {
                ...
                $foundOps++;
            } elseif (in_array($type, self::$_validOps)) {
                $foundOps++;
            }
