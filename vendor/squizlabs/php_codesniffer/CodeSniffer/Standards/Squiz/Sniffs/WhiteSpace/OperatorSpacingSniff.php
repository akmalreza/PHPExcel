...
class Squiz_Sniffs_WhiteSpace_OperatorSpacingSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if (($tokens[$stackPtr]['code'] === T_EQUAL
            || $tokens[$stackPtr]['code'] === T_MINUS) && isset($tokens[$stackPtr]['nested_parenthesis'])) {
        ...
        // Skip for '=&' case.
        if ($tokens[$stackPtr]['code'] === T_EQUAL && (isset($tokens[($stackPtr + 1)])
        ...
        if ($tokens[$stackPtr]['code'] === T_BITWISE_AND) {
            ...
            if ($tokens[($stackPtr - 1)]['code'] !== T_WHITESPACE) {
                ...
                $found = $tokens[($stackPtr - 2)]['line'] !== $tokens[$stackPtr]['line'] ? 'newline' : $tokens[($stackPtr - 1)]['length'];

                $phpcsFile->recordMetric($stackPtr, 'Space before operator', $found);
                if ($found !== 1
                    && ($found !== 'newline' || !$this->ignoreNewlines)
                ) {
        ...
        if ($tokens[$stackPtr]['code'] === T_MINUS || $tokens[$stackPtr]['code'] === T_PLUS) {
            ...
            if (isset(PHP_CodeSniffer_Tokens::$booleanOperators[$tokens[$prev]['code']])) {
                // Just trying to compare a negative value; eg. ($var || -1 === $b).
                return;
            }

            if (isset(PHP_CodeSniffer_Tokens::$assignmentTokens[$tokens[$prev]['code']])) {
            ...
            if (isset($invalidTokens[$tokens[$prev]['code']])) {
        ...
        if ($tokens[($stackPtr - 1)]['code'] !== T_WHITESPACE) {
            ...
            $phpcsFile->recordMetric($stackPtr, 'Space before operator', 0);
        } elseif (!isset(PHP_CodeSniffer_Tokens::$assignmentTokens[$tokens[$stackPtr]['code']])) {
