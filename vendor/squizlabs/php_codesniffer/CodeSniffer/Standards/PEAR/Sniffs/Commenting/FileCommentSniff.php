...
class PEAR_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        // Ignore vim header.
        if ($tokens[$commentStart]['code'] === T_COMMENT && strstr($tokens[$commentStart]['content'], 'vim:') !== false) {
    ...
    protected function processTags(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $commentStart)
    {
        ...
        $docBlock = $this instanceof \PEAR_Sniffs_Commenting_FileCommentSniff ? 'file' : 'class';
        ...
        foreach ($tokens[$commentStart]['comment_tags'] as $tag) {
            ...
            if ($this->tags[$name]['allow_multiple'] === false && isset($tagTokens[$name])) {
        ...
        foreach ($this->tags as $tag => $tagData) {
            ...
            $pos++;
            while (isset($foundTags[$pos]) && $foundTags[$pos] === $tag) {
                $pos++;
            }
        }//end foreach
    ...
    protected function processCopyright(PHP_CodeSniffer_File $phpcsFile, array $tags)
    {
        ...
        foreach ($tags as $tag) {
            ...
            $content = $tokens[($tag + 2)]['content'];
            $matches = array();
            if (preg_match('/^(\d{4})((.{1})(\d{4}))? (.+)$/', $content, $matches) !== 0) {
