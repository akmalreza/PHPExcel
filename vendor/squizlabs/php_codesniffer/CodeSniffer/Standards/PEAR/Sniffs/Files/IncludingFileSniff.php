...
class PEAR_Sniffs_Files_IncludingFileSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $inCondition = count($tokens[$stackPtr]['conditions']) !== 0;
        ...
        if (isset($tokens[$stackPtr]['nested_parenthesis'])) {
            foreach ($tokens[$stackPtr]['nested_parenthesis'] as $left => $right) {
                if (isset($tokens[$left]['parenthesis_owner'])) {
                    $inCondition = true;
                }
            }
        }
        ...
        if (isset(PHP_CodeSniffer_Tokens::$assignmentTokens[$tokens[$previous]['code']])) {
        ...
        } elseif ($tokenCode === T_INCLUDE_ONCE) {
