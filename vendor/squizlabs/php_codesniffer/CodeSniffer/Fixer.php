...
class PHP_CodeSniffer_Fixer
{
     ...
     */
    private ?\PHP_CodeSniffer_File $_currentFile = null;
     ...
     * @var array(int)
     */
    private array $_fixedTokens = array();
     ...
     * @var array(int => string)
     */
    private array $_oldTokenValues = array();
     ...
     * @var array()
     */
    private array $_changeset = array();

    /**
     * Is there an open changeset.
     */
    private bool $_inChangeset = false;

    /**
     * Is the current fixing loop in conflict?
     */
    private bool $_inConflict = false;

    /**
     * The number of fixes that have been performed.
     */
    private int $_numFixes = 0;
    ...
    public function startFile($phpcsFile)
    {
        ...
        foreach ($tokens as $index => $token) {
            $this->_tokens[$index] = isset($token['orig_content']) ? $token['orig_content'] : $token['content'];
        }
    ...
    public function fixFile()
    {
        ...
        while ($this->loops < 50) {
            ...
            if (PHP_CODESNIFFER_CBF === true && !$stdin) {
            ...
            if ($this->_numFixes === 0 && !$this->_inConflict) {
                // Nothing left to do.
                break;
    ...
    public function generateDiff($filePath=null, $colors=true)
    {
        ...
        $filename = strpos($filePath, $cwd) === 0 ? substr($filePath, strlen($cwd)) : $filePath;
    ...
    {
        if ($this->_inChangeset
            && isset($this->_changeset[$stackPtr])
        ) {
    ...
    public function replaceToken($stackPtr, $content)
    {
        ...
        if (!$this->_inChangeset
            && isset($this->_fixedTokens[$stackPtr])
        ) {
            $indent = "\t";
            if (!empty($this->_changeset)) {
                $indent .= "\t";
        ...
        if (PHP_CODESNIFFER_VERBOSITY > 1) {
            ...
            if (trim($this->_tokens[$stackPtr]) === '' && isset($this->_tokens[($stackPtr + 1)])) {
        ...
        if (!isset($this->_oldTokenValues[$stackPtr])) {
    ...
    {
        if (!isset($this->_fixedTokens[$stackPtr])) {
            return false;
        }
        ...
        if (PHP_CODESNIFFER_VERBOSITY > 1) {
            ...
            if (trim($this->_tokens[$stackPtr]) === '' && isset($tokens[($stackPtr + 1)])) {
                // Add some context for whitespace only changes.
                $append      = PHP_CodeSniffer::prepareForOutput($this->_tokens[($stackPtr + 1)]);
                $oldContent .= $append;
                $newContent .= $append;
    ...
    public function substrToken($stackPtr, $start, $length=null)
    {
        ...
        $newContent = $length === null ? substr($current, $start) : substr($current, $start, $length);
