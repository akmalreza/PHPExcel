...
class PHP_CodeSniffer_File
{
     ...
     */
    public ?\PHP_CodeSniffer $phpcs = null;

    /**
     * The Fixer object to control fixing errors.
     */
    public ?\PHP_CodeSniffer_Fixer $fixer = null;
     ...
     * The tokenizer being used for this file.
     */
    public string $tokenizerType = 'PHP';
     ...
     * @see getMetrics()
     */
    private array $_metrics = array();

    /**
     * Record the errors and warnings raised.
     */
    private bool $_recordErrors = true;
     ...
     * @var array()
     */
    private static array $_ignoredLines = array();
     ...
     * @var array()
     */
    private array $_ignoredListeners = array();
     ...
     * @var array()
     */
    private array $_ignoredCodes = array();
     ...
     * @var array()
     */
    private array $_listenerTimes = array();
    ...
    public function addTokenListener(PHP_CodeSniffer_Sniff $listener, array $tokens)
    {
        ...
        foreach ($tokens as $token) {
            ...
            if (!isset($this->_listeners[$token][$class])) {
    ...
    public function start($contents=null)
    {
        ...
        if ($this->_file === 'STDIN') {
            ...
            } elseif ($contents !== null && substr($contents, 0, 17) === 'phpcs_input_file:') {
        ...
        foreach ($this->_tokens as $stackPtr => $token) {
            ...
            if ($token['code'] === T_COMMENT
                ...
                || ($inTests && $token['code'] === T_INLINE_HTML)
            ) {
                if (strpos($token['content'], '@codingStandards') !== false) {
                    if (strpos($token['content'], '@codingStandardsIgnoreFile') !== false) {
                        ...
                        $this->_fixableCount = 0;
                        return;
                    } elseif (strpos($token['content'], '@codingStandardsChangeSetting') !== false) {
                        $start   = strpos($token['content'], '@codingStandardsChangeSetting');
                        $comment = substr($token['content'], ($start + 30));
                        $parts   = explode(' ', $comment);
                        if (count($parts) >= 3
                            && isset($this->phpcs->sniffCodes[$parts[0]])
                        ) {
            ...
            if (!isset($this->_listeners[$token['code']])) {
                continue;
            }

            foreach ($this->_listeners[$token['code']] as $listenerData) {
                if (isset($this->_ignoredListeners[$listenerData['class']])
                    || (isset($listenerIgnoreTo[$listenerData['class']])
                ...
                if (!isset($listenerData['tokenizers'][$this->tokenizerType])) {
                    continue;
                }
                ...
                if (PHP_CODESNIFFER_VERBOSITY > 2) {
                    ...
                    if (!isset($this->_listenerTimes[$this->_activeListener])) {
        ...
        if (!$foundCode && $this->tokenizerType === 'PHP' && $this->_file !== 'STDIN') {
    ...
    {
        if ($contents === null && !empty($this->_tokens)) {
        ...
        if (isset($this->phpcs->allowedFileExtensions[$extension])) {
        ...
        } elseif (isset($this->phpcs->defaultFileExtensions[$extension])) {
        ...
        try {
            ...
            if (defined('PHP_CODESNIFFER_IN_TESTS')) {
                $cliValues = $this->phpcs->cli->getCommandLineValues();
                if (isset($cliValues['tabWidth'])) {
                    $tabWidth = $cliValues['tabWidth'];
                }

                if (isset($cliValues['encoding'])) {
            ...
            if (PHP_CODESNIFFER_VERBOSITY > 0 || (PHP_CODESNIFFER_CBF === true && !$stdin)) {
        ...
        if (PHP_CODESNIFFER_VERBOSITY > 0 || (PHP_CODESNIFFER_CBF === true && !$stdin)) {
            $numLines = $this->numTokens === 0 ? 0 : $this->_tokens[($this->numTokens - 1)]['line'];
    ...
    public function addFixableError(
        ...
        return $recorded && $this->fixer->enabled;
    ...
    {
        if (isset(self::$_ignoredLines[$line])) {
            return false;
        }
        ...
        if (isset($this->_ignoredCodes[$sniffCode])) {
            return false;
        }

        // Make sure this message type has not been set to "warning".
        if (isset($this->ruleset[$sniffCode]['type'])
        ...
        } elseif ($this->phpcs->cli->errorSeverity === 0) {
        ...
        if (isset($this->ruleset[$sniffCode]['severity'])) {
        ...
        if (isset($this->ruleset[$sniffCode]['message'])) {
            $error = $this->ruleset[$sniffCode]['message'];
        }

        $message = empty($data) ? $error : vsprintf($error, $data);

        if (!isset($this->_errors[$line])) {
            $this->_errors[$line] = array();
        }

        if (!isset($this->_errors[$line][$column])) {
    ...
    private function _addWarning($warning, $line, $column, $code, $data, $severity, $fixable)
    {
        ...
        } elseif ($this->phpcs->cli->warningSeverity === 0) {
        ...
        $message = empty($data) ? $warning : vsprintf($warning, $data);

        if (!isset($this->_warnings[$line])) {
            $this->_warnings[$line] = array();
        }

        if (!isset($this->_warnings[$line][$column])) {
    ...
    {
        if (!isset($this->_metrics[$metric])) {
            $this->_metrics[$metric] = array(
                                        'values' => array(
                                                     $value => array($stackPtr),
                                                    ),
                                       );
        } elseif (!isset($this->_metrics[$metric]['values'][$value])) {
    ...
    public static function tokenizeString($string, $tokenizer, $eolChar='\n', $tabWidth=null, $encoding=null)
    {
        ...
        if (property_exists($tokenizer, 'skipMinified')
    ...
    private static function _createPositionMap(&$tokens, $tokenizer, $eolChar, $encoding, $tabWidth)
    {
        ...
        $checkEncoding = false;
        if ($encoding !== 'iso-8859-1' && function_exists('iconv_strlen')) {
            $checkEncoding = true;
        }
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            if (!isset(PHP_CodeSniffer_Tokens::$knownLengths[$tokens[$i]['code']])
            ...
            if (($tokens[$i]['code'] === T_COMMENT
                ...
                || ($inTests && $tokens[$i]['code'] === T_INLINE_HTML)) && strpos($tokens[$i]['content'], '@codingStandards') !== false
            ) {
                if (!$ignoring
    ...
    private static function _createTokenMap(&$tokens, $tokenizer, $eolChar)
    {
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            */
            ...
            if (isset(PHP_CodeSniffer_Tokens::$parenthesisOpeners[$tokens[$i]['code']])) {
    ...
    private static function _createParenthesisNestingMap(
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            } elseif (isset($tokens[$i]['parenthesis_closer'])
    ...
    private static function _createScopeMap(&$tokens, $tokenizer, $eolChar)
    {
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            if (isset($tokenizer->scopeOpeners[$tokens[$i]['code']])) {
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    $type    = $tokens[$i]['type'];
    ...
    private static function _recurseScopeMap(
        ...
        if (isset($tokenizer->scopeOpeners[$currType]['start'][$currType])) {
            $opener = $stackPtr;
        }
        ...
        for ($i = ($stackPtr + 1); $i < $numTokens; $i++) {
            ...
            if ($opener === null
                ...
                && isset($tokenizer->scopeOpeners[$currType]['end'][$tokenType])
            ) {
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    $type = $tokens[$stackPtr]['type'];
                    echo str_repeat("\t", $depth);
            ...
            if ($opener !== null
                ...
                if ($ignore > 0 && $tokenType === T_CLOSE_CURLY_BRACKET) {
                    ...
                    $ignore--;
                    continue;
                } elseif ($tokens[$opener]['code'] === T_OPEN_CURLY_BRACKET
                    && $tokenType !== T_CLOSE_CURLY_BRACKET) {
                    ...
                    if (($tokens[$stackPtr]['code'] === T_IF || $tokens[$stackPtr]['code'] === T_ELSEIF)
                        ...
                        if (!isset($tokens[$scopeCloser]['scope_opener'])) {
                            $validCloser = false;
                            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                echo str_repeat("\t", $depth);
                                echo "* closer is not valid (no opener found) *".PHP_EOL;
                            }
                        } elseif ($tokens[$tokens[$scopeCloser]['scope_opener']]['code'] !== $tokens[$opener]['code']) {
                            $validCloser = false;
                            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                echo str_repeat("\t", $depth);
                    ...
                    if ($validCloser) {
                        ...
                        if ($tokenizer->scopeOpeners[$tokens[$stackPtr]['code']]['shared'] === true) {
                            ...
                            && isset($tokenizer->scopeOpeners[$tokenType])) {
            ...
            if (isset($tokenizer->scopeOpeners[$tokenType])) {
                if ($opener === null) {
                    if ($tokenType === T_USE) {
                ...
                if (isset($tokens[$i]['scope_condition'])) {
                    ...
                    if (isset($tokenizer->scopeOpeners[$tokenType]['end'][T_CLOSE_CURLY_BRACKET])) {
                        $oldIgnore = $ignore;
                        $ignore    = 0;
                    }
                    ...
                    $oldDepth = $depth;
                    if ($isShared
                        && isset($tokenizer->scopeOpeners[$tokenType]['with'][$currType])
                    ) {
            ...
            if (isset($tokenizer->scopeOpeners[$currType]['start'][$tokenType])
                && $opener === null) {
                if ($tokenType === T_OPEN_CURLY_BRACKET) {
                    if (isset($tokens[$stackPtr]['parenthesis_closer'])
                        ...
                        for ($x = ($i - 1); $x > 0; $x--) {
                            if (isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[$x]['code']])) {
                                continue;
                            } else {
                                ...
                                if (isset($disallowed[$tokens[$x]['code']])) {
                                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                        echo str_repeat("\t", $depth);
                ...
                if (isset($tokens[$i]['parenthesis_owner'])) {
                    ...
                    if (isset(PHP_CodeSniffer_Tokens::$scopeOpeners[$tokens[$owner]['code']])
                ...
                && isset($tokenizer->scopeOpeners[$currType])) {
                ...
                if ($tokens[$i]['line'] >= ($startLine + 3)
                    && !isset(PHP_CodeSniffer_Tokens::$emptyTokens[$tokens[($i - 1)]['code']])
                ) {
                ...
                && $tokenType !== T_BREAK
                && isset($tokenizer->endScopeTokens[$tokenType])) {
                if (!isset($tokens[$i]['scope_condition'])) {
                    if ($ignore > 0) {
                        // We found the end token for the opener we were ignoring.
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
    ...
    private static function _createLevelMap(&$tokens, $tokenizer, $eolChar)
    {
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            if (isset($tokens[$i]['scope_condition'])) {
                ...
                } elseif ($lastOpener !== null && $tokens[$lastOpener]['scope_closer'] === $i) {
                    foreach (array_reverse($openers) as $opener) {
                        if ($tokens[$opener]['scope_closer'] === $i) {
                            ...
                            if ($condition !== $oldCondition) {
                                if (!isset($tokenizer->scopeOpeners[$oldCondition]['with'][$condition])) {
    ...
    public function isAnonymousFunction($stackPtr)
    {
        ...
        if (!isset($this->_tokens[$stackPtr]['parenthesis_opener'])) {
    ...
    public function getMethodParameters($stackPtr)
    {
        ...
        for ($i = $paramStart; $i <= $closer; $i++) {
            ...
            if (isset($this->_tokens[$i]['parenthesis_opener']) && $i !== $this->_tokens[$i]['parenthesis_closer']) {
                $i = ($this->_tokens[$i]['parenthesis_closer'] + 1);
            }

            // Don't do this if it's the close parenthesis for the method.
            if (isset($this->_tokens[$i]['bracket_opener']) && $i !== $this->_tokens[$i]['bracket_closer']) {
    ...
    public function getMethodProperties($stackPtr)
    {
        ...
        for ($i = ($stackPtr - 1); $i > 0; $i--) {
            if (!isset($valid[$this->_tokens[$i]['code']])) {
                break;
            }
    ...
    public function getMemberProperties($stackPtr)
    {
        ...
        if (!isset($this->_tokens[$ptr])
            ...
            if (isset($this->_tokens[$ptr])
                ...
                if (!isset($this->_tokens[$stackPtr]['nested_parenthesis'])
                    || empty($this->_tokens[$stackPtr]['nested_parenthesis'])
                ) {
    ...
    public function isReference($stackPtr)
    {
        ...
        if (isset(PHP_CodeSniffer_Tokens::$assignmentTokens[$this->_tokens[$tokenBefore]['code']])) {
        ...
        if (isset($this->_tokens[$stackPtr]['nested_parenthesis'])) {
            ...
            if (isset($this->_tokens[$lastBracket]['parenthesis_owner'])) {
        ...
        return $this->_tokens[$tokenAfter]['code'] === T_VARIABLE
    ...
    public function findStartOfStatement($start, $ignore=null)
    {
        ...
        for ($i = $start; $i >= 0; $i--) {
            if (isset($endTokens[$this->_tokens[$i]['code']])) {
            ...
            if (isset($this->_tokens[$i]['bracket_opener'])
                && $i === $this->_tokens[$i]['bracket_closer']) {
                $i = $this->_tokens[$i]['bracket_opener'];
            } elseif (isset($this->_tokens[$i]['parenthesis_opener'])
                && $i === $this->_tokens[$i]['parenthesis_closer']) {
            ...
            if (!isset(PHP_CodeSniffer_Tokens::$emptyTokens[$this->_tokens[$i]['code']])) {
                $lastNotEmpty = $i;
            }
        }//end for
    ...
    public function findEndOfStatement($start, $ignore=null)
    {
        ...
        for ($i = $start; $i < $this->numTokens; $i++) {
            if ($i !== $start && isset($endTokens[$this->_tokens[$i]['code']])) {
            ...
            // Skip nested statements.
            if (isset($this->_tokens[$i]['scope_closer'])
                && ($i === $this->_tokens[$i]['scope_opener']
                || $i === $this->_tokens[$i]['scope_condition'])) {
                $i = $this->_tokens[$i]['scope_closer'];
            } elseif (isset($this->_tokens[$i]['bracket_closer'])
                && $i === $this->_tokens[$i]['bracket_opener']) {
                $i = $this->_tokens[$i]['bracket_closer'];
            } elseif (isset($this->_tokens[$i]['parenthesis_closer'])
                && $i === $this->_tokens[$i]['parenthesis_opener']) {
                $i = $this->_tokens[$i]['parenthesis_closer'];
            }
    ...
    public function findFirstOnLine($types, $start, $exclude=false, $value=null)
    {
        ...
        for ($i = $start; $i >= 0; $i--) {
            ...
            foreach ($types as $type) {
                ...
                } elseif ($this->_tokens[$i]['code'] === $type) {
                    $found = false;
                    break;
                }
            }
    ...
    public function hasCondition($stackPtr, $types)
    {
        ...
        if (!isset($this->_tokens[$stackPtr])) {
            return false;
        }

        // Make sure the token has conditions.
        if (!isset($this->_tokens[$stackPtr]['conditions'])) {
            return false;
        }
    ...
    public function findExtendedClassName($stackPtr)
    {
        ...
        if (!isset($this->_tokens[$stackPtr]['scope_closer'])) {
            return false;
        }
